...
public class PImage {
	...
	static Color magenta = new Color(255, 0, 191);
	private int widht, height;
	private ArrayList<BufferedImage> images = new ArrayList<>();
	
	public PImage(BufferedImage img){
		...
		//BufferedImage g = new BufferedImage(286, 268, BufferedImage.TYPE_INT_ARGB);  //debug picture
		...
		for(int y = 0; y<height;y++)
			for(int x = 0; x< widht; x++)
				if(new Color(img.getRGB(x, y)).equals(magenta))
					wall[x][y] = true;
		...
		//g.getGraphics().drawImage(getImageById(48), 0, 0, Color.white, null);
		//Display d = new Display(800, 800, "testing", g);
	}
